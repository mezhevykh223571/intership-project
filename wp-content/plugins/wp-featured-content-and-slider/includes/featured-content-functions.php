...
function wpfcas_setup_post_types() {
	...
	$wpfcas_labels =  apply_filters( 'wpfcas_labels', array(
		...
		'parent_item_colon'   => '',
		'menu_name'           => __('Our Members Content', 'featured_post'),
...
function wpfcas_add_meta_box() {
	add_meta_box('custom-metabox',__( 'Our Members Content Settings', 'wp-featured-content-and-slider' ),'wpfcas_box_callback', array(WPFCAS_POST_TYPE, 'post') );
}
...
//	$read_more_link = isset($_POST['wpfcas_slide_link']) ? stripslashes_deep($_POST['wpfcas_slide_link']) : '';
	...
	$instrument     = isset($_POST['instrument'])        ? stripslashes_deep($_POST['instrument']) : '';
	$facebookLink   = isset($_POST['facebook-link'])     ? stripslashes_deep($_POST['facebook-link']) : '';
	$twitterLink    = isset($_POST['twitter-link'])      ? stripslashes_deep($_POST['twitter-link']) : '';
	$googleplusLink = isset($_POST['googleplus-link'])   ? stripslashes_deep($_POST['googleplus-link']) : '';
	$facebook       = isset($_POST['facebook'])          ? stripslashes_deep($_POST['facebook']) : '';
	$twitter        = isset($_POST['twitter'])           ? stripslashes_deep($_POST['twitter']) : '';
	$googleplus     = isset($_POST['googleplus'])        ? stripslashes_deep($_POST['googleplus']) : '';

//	update_post_meta($post_id, 'wpfcas_slide_icon', $featured_icon);
//    update_post_meta($post_id, 'wpfcas_slide_link', $read_more_link);

	update_post_meta($post_id, 'instrument', $instrument);
	update_post_meta($post_id, 'facebook-link', $facebookLink);
	update_post_meta($post_id, 'twitter-link', $twitterLink);
	update_post_meta($post_id, 'googleplus-link', $googleplusLink);
